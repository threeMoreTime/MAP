...
```mermaid
flowchart TD
  A[MetroMan.cn] -->|scrapers/scrape_all_cities.py| B[cities/ 城市线路图/规划图 PNG]
  C[MetroDB.org] -->|scrapers/scrape_metrodb.py| D[cities/ 城市客流 stats JSON]
  D -->|scrapers/generate_charts.py| E[output/ 年度趋势图/全国图表 PNG]
  D -->|数据嵌入| F[dashboard.html 内嵌 DATA]
  B --> F
...
|------|------|
| **数据源** | MetroMan.cn |
| **执行脚本** | `scrapers/scrape_all_cities.py` |
| **输入** | MetroMan.cn 城市页面 URL（硬编码或配置） |
| **输出** | 各城市地铁线路图/规划图，PNG 格式，存储于 `cities/{city}/` 目录 |
| **覆盖范围** | 全部 50 个城市 |
| **依赖** | 网络连接、目标站点可访问 |
...
|------|------|
| **数据源** | MetroDB.org |
| **执行脚本** | `scrapers/scrape_metrodb.py` |
| **输入** | MetroDB.org 城市客流页面 URL |
| **输出** | 各城市客流统计数据，JSON 格式（`cities/{city}/{city}_stats.json`） |
| **覆盖范围** | 34 个城市有数据，16 个城市标记为"无数据" |
| **依赖** | 网络连接、目标站点可访问 |
...
| 项目 | 说明 |
|------|------|
| **执行脚本** | `scrapers/generate_charts.py` |
| **输入** | 阶段二输出的 JSON 客流数据（`cities/{city}/{city}_stats.json`） |
| **输出** | 年度趋势图（`cities/{city}/{city}_yearly_trend.png`）、全国对比图（`output/national_comparison.png`）、总览仪表盘（`output/overview_dashboard.png`） |
| **依赖** | matplotlib、阶段二的 JSON 数据 |

**说明：** 读取 JSON 数据，使用 matplotlib 生成城市维度的年度客流趋势折线图和全国层面的对比图表。生成的 PNG 图片可嵌入大屏或用于报告。

### 阶段四：数据嵌入（JSON → dashboard.html）

| 项目 | 说明 |
|------|------|
| **操作方式** | 将 JSON 数据以 JavaScript 变量形式直接内嵌到 dashboard.html |
...
**关键约束：**
- `generate_charts.py` 依赖 `scrape_metrodb.py` 的输出，必须先完成客流数据采集
...
在 `cities/` 城市目录基础上，建立统一的 `data/latest` 数据层，为后续 Dashboard 数据外置（Phase 4）提供标准化接口。
...
| `data/latest/city_assets_index.json` | 索引 50 个城市的资源文件（PNG/JSON） |
