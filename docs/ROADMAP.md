...
### Phase 6：增量更新与定期采集（已完成远端 dry-run 验证）
...
**状态**：Phase 6 数据增量更新 workflow 已完成远端 dry-run 验证；write / 自动 PR 创建待后续按需验证。
...
### Phase 8：城市对比功能 (City Compare)

**目标**：新增城市横向对比页面，支持 2-5 城市运营指标与数据完整度对比。

**产物**：
- [x] **城市对比页面**：新增 `/#/compare` 路由，支持 URL 参数 `?cities=...` 保存选择状态
- [x] **城市选择器**：中文名/拼音搜索，键盘操作，默认自动选取完整度高且有日客流的前 3 个城市
- [x] **指标概览卡片**：日客流、运营里程、站点数、线路数、完整度评分，缺失值中性展示
- [x] **对比图表**：条形图（6 指标切换）、雷达图（多维归一化）、年度趋势折线图
- [x] **数据完整度对比区**：评分、等级、资源状态、缺失项
- [x] **详细对比表**：桌面端表格 / 移动端纵向卡片
- [x] **验收测试 T32/T33/T34**：基础加载、城市选择与指标、375px 移动端无溢出
- [x] **线上 smoke 覆盖 /#/compare**

**验收标准**：
- `test:ui`：T01-T34，Total 34，PASS 33，FAIL 0，MANUAL 1
- `npm run typecheck` 和 `npm run build` 通过
- 375px 无横向溢出

**状态**：Phase 8 城市对比功能实现完成（待远端 CI/CD 与 Pages smoke 全绿后最终收口）。
