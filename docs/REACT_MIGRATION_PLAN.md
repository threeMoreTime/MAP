...
| 5.5 | 城市详情页视觉验收与响应式细节收口（T25 边界状态测试、微调优化） | 已完成 |
...
### 6.15 Phase 5.5 城市详情页视觉验收与响应式细节收口

Phase 5.5 对城市详情页进行视觉验收和响应式细节收口：

- **视觉验收**：使用 cursor-ide-browser MCP 在 1440x810 和 375x812 视口下检查 xiamen/beijing/hohhot/foshan 四个城市的详情页
- **查看器高度调整**：PC 端 680→620px、平板 560→520px、移动端 400→380px，避免首屏被巨大地图占据
- **Hero padding 收紧**：从 36px/40px 调整为 32px/36px
- **CitySourceInfo 紧凑化**：减小 padding/gap/font-size，使信息面板更紧凑
- **数据说明卡片 max-width**：添加 max-width: 800px 避免过宽空洞
- **T25 验收测试**：新增边界城市状态测试（xiamen 完整资源、hohhot 部分缺失、foshan 全缺失、375px 移动端）
