...
> **当前状态**：Phase 5.1/5.6 已通过远端全绿 CI/CD 与线上冒烟验证。  
...
| :--- | :--- | :---: | :---: | :--- | :--- |
| **Phase 5.6.1** | 数据覆盖口径文案统一 | **P0** | 否 | 消除 34/23/27 数据指标的不一致误解，统一全网文案口径。 | `test:ui` 增强断言校验 + 线上 Smoke Test |
| **Phase 5.7** | 旧版基线维护策略落纸 | **P0** | 否 | 明确 `dashboard.html` 归档与回归政策，规范双前端并存策略。 | 文档合规审查 + CI `legacy-check` |
| **Phase 6** | 数据增量更新与定期采集 | **P1** | 是 | 建立可控的 GitHub Actions 定期数据采集与变更自动 PR 机制。 | 模拟采集 Run & 自动数据格式校验 |
...
## 5. Phase 5.7：旧版 dashboard.html 维护策略
