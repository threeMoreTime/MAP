...
| **Phase 6** | 数据增量更新与定期采集 | **P1** | 是 | 【已完成远端 dry-run 验证】建立可控的 GitHub Actions 定期数据采集与变更自动 PR 机制；write / 自动 PR 待后续按需验证。 | 模拟采集 Run & 自动数据格式校验 |
| **Phase 6.1** | 数据质量报告与异常检测 | **P1** | 否 | 【已完成】为数据管道建立多维质量评分，产出质量报告并挂载在构建流。 | `npm run check:static` 自动化套件扩展 |
| **Phase 7** | 数据质量中心 (Data Quality Center) | **P1** | 是 | 【已完成】新建 `/#/data-quality` 页面，展示完整度大纲、缺失索引与响应式数据表。 | 新增 `test:ui` T29/T30/T31 + 线上 Smoke |
| **Phase 8** | 多城市数据交叉对比功能 | **P2** | 是 | 【已实现】新建 `/#/compare` 模块，支持 2-5 个城市的多指标横向对比分析。 | 新增 `test:ui` T32/T33/T34 + 线上 Smoke |
...
## 6. Phase 6：数据增量更新与定期采集（已完成远端 dry-run 验证）
