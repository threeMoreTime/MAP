...
CitiesPage 页面加载时先 fetch `assets/city-covers/manifest.json`，只收集 `status === 'downloaded' && file` 的城市封面 URL 到 `coverMap`。manifest 加载失败时 coverMap 为空，所有城市走渐变回退。

城市卡片封面使用 CSS `background-image` 叠加策略：

```tsx
// manifest-aware: 仅 status=downloaded 且有 file 的城市生成封面 URL
...
- **manifest downloaded 城市**：浏览器显示 webp 图片
- **manifest fallback 城市**（如 hohhot）：CSS 渐变色回退，`data-has-cover="false"`
- **manifest 加载失败**：所有城市走 CSS 渐变，console.warn
...
Phase 4.7.2 验证并确认城市卡片封面图在前端的实际渲染效果：

**自动化验收（T20）：**
- 验收脚本新增 T20 测试项，检查 `/#/cities` 页面封面图资源加载（manifest-aware）
- 通过 `data-has-cover` 属性统计：总数 50，`true` 应为 49，`false` 应为 1
- 检查至少一个真实 cover backgroundImage 包含 city-covers
- 从真实 cover 中提取 URL，fetch 验证 response.status === 200 且 content-type 包含 image/
- 专门检查 hohhot：`data-city="hohhot"` 的元素 `data-has-cover="false"`，backgroundImage 不包含 hohhot.webp

**CSS 层级结构：**
- `.city-cover-image`（底层）：`url(封面图), linear-gradient(渐变)` — 图片优先，渐变 fallback
- `.city-cover-art`（中层）：`radial-gradient` 装饰层（rgba 0.06 透明度，不影响图片可见性）
- `.city-cover-overlay`（上层）：底部渐变遮罩（z-index: 1），从底部深色到顶部透明
- `.city-cover-info`（最上层）：城市名和线路信息（z-index: 2）

**渲染行为：**
- 有封面图城市：显示真实城市天际线图片
- 无封面图城市（如呼和浩特）：静默回退到 CSS 渐变色
- Hover 时封面区 scale(1.10) 放大动效
- 底部遮罩保证文字始终清晰可读
